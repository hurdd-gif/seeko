...
/* ─────────────────────────────────────────────────────────
 * ANIMATION STORYBOARD — Profile Menu (identity)
 *
 ...
 *
 *  This is the identity half of the top-bar chrome. Navigation lives in the
 *  sibling MoreMenu; this menu only carries who-you-are + account actions.
 * ───────────────────────────────────────────────────────── */
...
export function ProfileMenu({ email, displayName, avatarUrl }: ProfileMenuProps) {
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            ...
            {/* Account links */}
            <div className="py-1">
              <ProfileLink href="/settings" icon={Settings} label="Settings" onClick={() => setOpen(false)} />
            </div>
...
function ProfileLink({ href, icon: Icon, label, onClick }: { href: string; icon: React.ElementType; label: string; onClick: () => void }) {
  return (
    <motion.div whileHover={{ x: 2 }} transition={springs.snappy}>
      <Link
        href={href}
        onClick={onClick}
