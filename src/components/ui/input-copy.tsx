...
interface InputCopyProps extends Omit<HTMLAttributes<HTMLDivElement>, "children"> {
  ...
  /** Disable the floating tooltip when rendered inside clipped containers. */
  showTooltip?: boolean;
  /** Dense inline mode for table/list rows where the hover shell must not widen the row. */
  compact?: boolean;
  /** Use monospace for values such as emails and IDs. */
  monospace?: boolean;
}

const InputCopy = forwardRef<HTMLDivElement, InputCopyProps>(
  ({ value, label, onCopy, disabled, variant = "icon", align = "right", showTooltip = true, compact = false, monospace = true, className, ...props }, ref) => {
    ...
    const valueElement = (
      <span
        ...
        style={{ fontVariationSettings: fontWeights.normal }}
      >
        <mark className="bg-transparent text-inherit transition-colors duration-150">
          {value}
        </mark>
      </span>
    );
    ...
    return (
      <div
        ref={ref}
        className={cn(
          compact ? "inline-flex max-w-full align-middle" : "flex flex-col gap-0.5",
