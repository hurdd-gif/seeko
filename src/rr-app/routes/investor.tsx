    ...
    <div className="flex flex-col gap-6">
      ...
      <FadeRise delay={delay(TIMING.progress)} y={6}>
        <div className="grid items-stretch gap-4 lg:grid-cols-[minmax(0,1.04fr)_minmax(0,0.96fr)]">
          ...
          <InvestorWhereWereGoing areas={dashboardAreas} tasksPerArea={tasksPerArea} />
        </div>
      </FadeRise>
...
/** Inline health chip — replaces the old full-width tinted strip. */
function HealthChip({
  riskCount,
  blocked,
  overdue,
  ...
  const hasIssues = riskCount > 0;
  return (
    <span
      className="inline-flex w-fit shrink-0 items-center gap-2 rounded-full px-3 py-1.5 text-[12.5px] font-medium leading-[17px]"
      style={{
        backgroundColor: hasIssues ? 'rgba(255, 110, 94, 0.09)' : 'rgba(0, 0, 0, 0.04)',
        color: hasIssues ? '#b23b2c' : '#505050',
      }}
    >
      <span
        aria-hidden
        className="size-1.5 rounded-full"
        style={{ backgroundColor: hasIssues ? '#d4503e' : '#34a853' }}
      />
      {hasIssues ? (
...
/** Micro line+area sparkline. Stretched to fill (preserveAspectRatio=none)
 *  with a non-scaling stroke so the trend shape reads without a fixed box. */
function Sparkline({ values }: { values: number[] }) {
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none" className="h-8 w-full" aria-hidden>
      <path d={area} fill="#1f1f1f" fillOpacity="0.06" />
      <path
        d={line}
        fill="none"
        stroke="#1f1f1f"
...
/** Monthly spend (NOT cumulative) over a trailing window, oldest→newest, for
 *  the burn sparkline. Real paid disbursements bucketed by calendar month;
 *  months with no payments read as 0 so the trend reflects actual cadence. */
function buildMonthlyBurnSeries(
  payments: InvestorPaymentsData['payments'],
  months = 6,
): number[] {
  const paid = payments.filter((payment) => payment.paidAt);
...
function InvestorKpiStrip({
  ...
  const burnSeries = useMemo(
    () => buildMonthlyBurnSeries(payments?.payments ?? []),
    [payments],
  );

  // Footer fallback copy when there isn't enough history for a sparkline.
  const burnSub = !burn
  ...
  // then a fixed-height encoding band — but the encoding type is chosen per
  // metric so it stays honest: only burn has a real time-series (sparkline);
  // the rest get a gauge / ratio ticks / status dots, never a faked trend.
  const cards: {
    label: string;
    value: string;
    valueClass?: string;
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
      {cards.map((card) => (
        <div key={card.label} className={`${CARD} flex flex-col px-5 py-4`}>
    ...
    <section className={`${CARD} overflow-hidden`}>
      <div className="flex flex-wrap items-end justify-between gap-x-4 gap-y-2 px-6 pb-1 pt-5">
        <div>
          ...
          <h2 className="mt-1 text-[26px] font-semibold leading-[30px] tabular-nums text-[#111]">
...
function InvestorProgressPanel({
  ...
  return (
    <section className={`${CARD} flex min-w-0 flex-col overflow-hidden`}>
      ...
      {areas.length === 0 ? (
        <p className="px-6 py-10 text-center text-[13px] text-[#8a8a8a]">Progress data will appear here.</p>
      ) : (
        <div className="grid flex-1 place-items-center gap-x-4 gap-y-7 px-6 py-7 [grid-template-columns:repeat(auto-fit,minmax(132px,1fr))]">
